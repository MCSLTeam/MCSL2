...
class MojangEula:
    ...
    def __init__(self):
        self.serverDir = f"./Servers/{serverVariables.serverName[-1]}"
...
class ServerLauncher:
    ...
    def reGetNewJava(self):
        self.javaPath = serverVariables.javaPath[-1]

    def setjvmArg(self):
        """生成开服命令参数"""
        self.jvmArg = [
            f"-Xms{serverVariables.minMem[-1]}{serverVariables.memUnit[-1]}",
            f"-Xmx{serverVariables.maxMem[-1]}{serverVariables.memUnit[-1]}",
        ]
        # add jvm args
        if isinstance(serverVariables.jvmArg, list):
            for arg in serverVariables.jvmArg:
                self.jvmArg.extend(arg)
        else:
            if serverVariables.jvmArg:
                for arg in serverVariables.jvmArg:
                    self.jvmArg.append(arg)
...
class MinecraftServerResMonitorUtil(QObject):
    ...
    def getServerMem(self):
        ...
        divisionNum = divisionNumList[serverVariables.memUnit[-1]]
        try:
