...
class Util:
    ...
    def loadInstallProfile(text: str):
        ...
        if profile_name.lower() in ["neoforge", "forge"]:
            print(f"Loading {profile_name} install profile (spec version: {spec})")

        # NeoForge and modern Forge both use spec 1
        # Older Forge versions use spec 0
        if spec == 0:
            return InstallV1.from_installV0(Install.of(profile))
        elif spec == 1:
            return InstallV1.of(profile)
        else:
            # Handle future spec versions by defaulting to InstallV1
            print(f"Warning: Unknown spec version {spec}, treating as spec 1")
            return InstallV1.of(profile)
