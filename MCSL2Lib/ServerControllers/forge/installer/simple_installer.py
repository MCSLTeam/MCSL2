...
class SimpleInstaller:
    ...
    async def installServer(
        ...
        with BytesIO(installer.read_bytes()) as installerBuf:
            try:
                ...
                raise Exception(f"Failed to load install profile: {e}")
            serverInstaller = ServerInstall(profile, installer, monitor)
            return await serverInstaller.run(targetDir, java, self.detailed)
