...
class ShellArchives(QWidget):
    def __init__(self):
        ...
        self.shellArchivesTitle.setText(self.tr("导入 含开服脚本的 完整的 服务器 压缩包/文件夹"))
        self.shellArchivesImportStatusText.setText(self.tr("[状态文本]"))
        self.shellArchivesImportArchives.setText(self.tr("导入文件"))
        self.shellArchivesImportFolder.setText(self.tr("导入文件夹"))
        self.shellArchivesImportTitle.setText(self.tr("1. 导入文件/文件夹"))
        self.shellArchivesSelectShellStatusText.setText(self.tr("[状态文本]"))
        self.shellArchivesSelectShellTitle.setText(self.tr("2.选择开服脚本"))
        self.shellArchivesValidateArgsAutoDetectJavaPrimaryPushBtn.setText(self.tr("自动查找Java"))
        self.shellArchivesValidateArgsJavaSubtitleLabel.setText(self.tr("Java:"))
        self.shellArchivesValidateArgsJavaListPushBtn.setText(self.tr("Java列表"))
        self.shellArchivesValidateArgsManuallyAddJavaPrimaryPushBtn.setText(self.tr("手动导入"))
        self.shellArchivesValidateArgsDownloadJavaPrimaryPushBtn.setText(self.tr("下载Java"))
        self.shellArchivesValidateArgsOutputDeEncodingLabel.setText(
            self.tr("控制台输出编码（优先级高于全局设置）")
        )
        self.shellArchivesValidateArgsDeEncodingSubtitleLabel.setText(self.tr("编码设置："))
        self.shellArchivesValidateArgsInputDeEncodingLabel.setText(self.tr("指令输入编码（优先级高于全局设置）"))
        self.shellArchivesValidateArgsJVMArgPlainTextEdit.setPlaceholderText(
            self.tr("可选，用一个空格分组")
        )
        self.shellArchivesValidateArgsJVMArgSubtitleLabel.setText(self.tr("JVM参数："))
        self.shellArchivesValidateArgsMemSubtitleLabel.setText(self.tr("内存:"))
        self.shellArchivesValidateArgsToSymbol.setText(self.tr("~"))
        self.shellArchivesValidateArgsTitle.setText(self.tr("3. 确认参数"))
        self.shellArchivesValidateArgsDownloadCorePrimaryPushBtn.setText(self.tr("下载核心"))
        self.shellArchivesValidateArgsCoreSubtitleLabel.setText(self.tr("核心："))
        self.shellArchivesValidateArgsManuallyAddCorePrimaryPushBtn.setText(self.tr("重新导入"))
        self.shellArchivesSaveTitle.setText(self.tr("4. 完成导入"))
        self.shellArchivesSaveServerNameLineEdit.setPlaceholderText(self.tr("设置服务器昵称，不能包含非法字符"))
        self.shellArchivesSaveSaveServerPrimaryPushBtn.setText(self.tr("导入！"))
